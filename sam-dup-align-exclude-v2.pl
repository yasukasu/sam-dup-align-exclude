...
# v2 supprot sam files produced with '--no-unal' option for bowtie2
# under construction
...
while(<IN>){
  ...
  if($Rn==1){
    ...
    #if($id_R1 ne $id_R2){die "Error in input of pair end reads. R1:$id_R1, R2:$id_R2 (line:$read_n)";}
    #print "$id_R2 => $Rn\n";

    if($id_R1 ne $id_R2){
      ($id_R1, $flag_R1, $AS_R1, $XS_R1) = ($id_R2, $flag_R2, $AS_R2, $XS_R2);
      next;
    }
...
print "Number of inputted clusters   :\t", $read_n, "\n";
print "Number of All concordant align.:\t", $out_all,  " (", sprintf("%.2f",$out_all/$read_n*100), "%)\n";
print " > no multipe aligment (no XS tag in either R1 or R2):\t", $out_noXS,  " (", sprintf("%.2f",$out_noXS/$read_n*100), "%)\n";
print " > multiple aligments  (XS>AS tag in either R1 or R2):\t", $out_1XS,  " (", sprintf("%.2f",$out_1XS/$read_n*100), "%)\n";
print "   => file: $file_out_ex total: ", $out_noXS+$out_1XS, " (", sprintf("%.2f",($out_noXS+$out_1XS)/$read_n*100), "%) \n";
print " > duplicated align (AS=XS in both reads)            :\t", $out_dup, " (", sprintf("%.2f",$out_dup/$read_n*100), "%)\n";
